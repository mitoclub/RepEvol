...
# folder = "FOUND4_TXT"
# group = "Mitochondria_GreatApes.muscle.sequester PLUS HOMO SAPIENS.fasta"
...
folder = "FOUND3_TXT"
group = "SupraHominida.muscle.sequester.fasta"
...
for f in files:
    ...
    while True:
        ...
        ##### First start 
        ######### mapfile = open(paz,'r')
        con = sqlite3.connect(paz)
        cur = con.cursor()

        cur.execute('SELECT * FROM species WHERE ori=%s' % read[0])
        good=cur.fetchall()
        read[0]=str(good[0][2])


        cur.execute('SELECT * FROM species WHERE ori=%s' % read[1])
        #print(read[1])
        good=cur.fetchall()
        read[1]=str(good[0][2])
